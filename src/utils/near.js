...
export const completeNearTx = async ({
  ...
  updateOverlay({
    overlayMessage: (
      <>
        <a
          href={`https://testnet.nearblocks.io/txns/` + result.transaction.hash}
          target="_blank"
        >
          Explorer Link
        </a>
        ...
        Object.entries(js2coreActions).forEach(([k, v]) => {
          ...
          switch (v) {
            case 'AddKey':
              tx.actions.push({
                [v]: {
                  public_key: oldAction[k].publicKey.toString(),
                  access_key: {
                    nonce: '0',
                    permission: 'FullAccess',
                  },
    ...
    if (v === 'actions') {
      for (const action of transaction[v]) {
        ...
        Object.entries(core2jsActions).forEach(([k, v]) => {
          ...
          if (v === 'addKey') {
            action[v] = {
              publicKey: PublicKey.fromString(action[v].public_key),
              accessKey: {
                nonce: 0,
                permission: {
                  fullAccess: {},
